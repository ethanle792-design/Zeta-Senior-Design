#!/usr/bin/env python3
"""
iq_logger_compass_cs16.py

Same behavior as iq_logger_compass.py but stores IQ as CS16
(interleaved int16 I/Q) instead of CF32.

Output files:
  <base>.cs16
  <base>.json
  <base>_heading.csv
"""

import argparse
import csv
import json
import math
import os
import signal
import sys
import time
import threading
from dataclasses import dataclass
from datetime import datetime, timezone

import numpy as np

import SoapySDR
from SoapySDR import SOAPY_SDR_RX, SOAPY_SDR_CF32
from smbus2 import SMBus


STOP = False


def _sigint_handler(signum, frame):
    global STOP
    STOP = True

def utc_ts():
    return datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%SZ")


def mkdir(path):
    os.makedirs(path, exist_ok=True)


def wrap360(deg: float) -> float:
    deg %= 360.0
    if deg < 0:
        deg += 360.0
    return deg


def parse_args():
    p = argparse.ArgumentParser()

    p.add_argument("--device", default="driver=bladerf")
    p.add_argument("--channel", type=int, default=0)

    p.add_argument("--freq", type=float, required=True)
    p.add_argument("--rate", type=float, required=True)
    p.add_argument("--gain", type=float)
    p.add_argument("--bandwidth", type=float)

    group = p.add_mutually_exclusive_group(required=True)
    group.add_argument("--duration", type=float)
    group.add_argument("--samples", type=int)

    p.add_argument("--out", default="./captures")
    p.add_argument("--name")

    p.add_argument("--block", type=int, default=262144)
    p.add_argument("--timeout-us", type=int, default=250000)

    p.add_argument("--compass", action="store_true")
    p.add_argument("--compass-bus", type=int, default=7)
    p.add_argument("--compass-addr", type=lambda x: int(x, 0), default=0x2C)
    p.add_argument("--compass-hz", type=float, default=30)

    # Hard-coded defaults from latest compass_cal.json
    p.add_argument("--x-offset", type=float, default=55.5)
    p.add_argument("--y-offset", type=float, default=-1403.5)
    p.add_argument("--x-scale", type=float, default=1.004128)
    p.add_argument("--y-scale", type=float, default=0.995906)
    p.add_argument("--compass-alpha", type=float, default=0.7)  

    return p.parse_args()


def s16(lo, hi):
    v = (hi << 8) | lo
    return v - 65536 if v & 0x8000 else v


def read_xyz(bus, addr):
    # Match compass_cardinal_actual.py trigger behavior
    bus.write_byte_data(addr, 0x09, 0x01)
    bus.write_byte_data(addr, 0x0A, 0x01)
    bus.write_byte_data(addr, 0x0B, 0x01)

    time.sleep(0.03)

    b = bus.read_i2c_block_data(addr, 0x00, 7)

    x = s16(b[1], b[2])
    y = s16(b[3], b[4])
    z = s16(b[5], b[6])

    return b[0], x, y, z

def deg_to_cardinal_16(deg: float) -> str:
    # 16-wind compass rose
    dirs = ["N","NNE","NE","ENE","E","ESE","SE","SSE",
            "S","SSW","SW","WSW","W","WNW","NW","NNW"]
    i = int((deg + 11.25) // 22.5) % 16
    return dirs[i]

@dataclass
class HeadingState:
    heading: float | None = None
    status: str = "DISABLED"

class CompassThread(threading.Thread):

    def __init__(self, args, csv_path, state, start_t, stop_event):
        super().__init__(daemon=True)

        self.args = args
        self.csv_path = csv_path
        self.state = state
        self.t0 = start_t
        self.stop_event = stop_event

        self.hx = None
        self.hy = None

    def run(self):
        with SMBus(self.args.compass_bus) as bus, open(self.csv_path, "w", newline="") as f:
            w = csv.writer(f)
            w.writerow(["t", "status", "heading", "x_raw", "y_raw", "z_raw"])

            period = 1 / self.args.compass_hz

            while not self.stop_event.is_set():
                t = time.monotonic() - self.t0

                try:
                    st, x_raw, y_raw, z_raw = read_xyz(bus, self.args.compass_addr)

                    # Calibrate using corrected hard-coded values
                    x = (x_raw - self.args.x_offset) * self.args.x_scale
                    y = (y_raw - self.args.y_offset) * self.args.y_scale

                    # Match corrected orientation handling
                    y = -y

                    theta = math.atan2(y, x)
                    hx = math.cos(theta)
                    hy = math.sin(theta)

                    if self.hx is None:
                        self.hx, self.hy = hx, hy
                    else:
                        a = self.args.compass_alpha
                        self.hx = a * self.hx + (1 - a) * hx
                        self.hy = a * self.hy + (1 - a) * hy

                        r = math.hypot(self.hx, self.hy)
                        if r > 1e-9:
                            self.hx /= r
                            self.hy /= r

                    # Match compass_cardinal_actual.py correction
                    heading = math.degrees(math.atan2(self.hy, self.hx)) - 90
                    heading = wrap360(heading)

                    self.state.heading = heading
                    self.state.status = "OK"

                    w.writerow([f"{t:.4f}", self.state.status, f"{heading:.2f}", deg_to_cardinal_16(heading), x_raw, y_raw, z_raw])

                except Exception as e:
                    self.state.status = str(e)
                    w.writerow([f"{t:.4f}", "ERR", "", "", "", "",""])

                f.flush()
                time.sleep(period)


def main():
    global STOP
    signal.signal(signal.SIGINT, _sigint_handler)

    args = parse_args()

    mkdir(args.out)

    base = args.name or f"capture_{utc_ts()}"

    iq_path = os.path.join(args.out, base + ".cs16")
    json_path = os.path.join(args.out, base + ".json")
    heading_csv = os.path.join(args.out, base + "_heading.csv")

    print("[+] Opening SDR")

    sdr = SoapySDR.Device(args.device)

    ch = args.channel

    sdr.setSampleRate(SOAPY_SDR_RX, ch, args.rate)
    sdr.setFrequency(SOAPY_SDR_RX, ch, args.freq)

    if args.bandwidth is not None:
        try:
            sdr.setBandwidth(SOAPY_SDR_RX, ch, args.bandwidth)
        except Exception:
            pass

    if args.gain is not None:
        sdr.setGain(SOAPY_SDR_RX, ch, args.gain)

    rx_stream = sdr.setupStream(SOAPY_SDR_RX, SOAPY_SDR_CF32, [ch])
    sdr.activateStream(rx_stream)

    buf = np.empty(args.block, np.complex64)

    if args.samples:
        target = args.samples
    else:
        target = int(args.duration * args.rate)

    written = 0

    state = HeadingState()
    stop_event = threading.Event()

    ct = None

    # TODO: mark timestamp of start of compass logging to put into json
    if args.compass:
        ct = CompassThread(args, heading_csv, state, time.monotonic(), stop_event)
        ct.start()

    # TODO: mark timestamp of start of IQ logging to put into json
    with open(iq_path, "wb", buffering=0) as f:
        while not STOP and written < target:
            need = min(args.block, target - written)

            sr = sdr.readStream(rx_stream, [buf], need, timeoutUs=args.timeout_us)

            if sr.ret > 0:
                iq = buf[:sr.ret]

                i = np.clip(iq.real * 32767, -32768, 32767).astype(np.int16)
                q = np.clip(iq.imag * 32767, -32768, 32767).astype(np.int16)

                inter = np.empty(i.size * 2, np.int16)
                inter[0::2] = i
                inter[1::2] = q

                f.write(inter.tobytes())
                written += sr.ret

    stop_event.set()
    if ct is not None:
        ct.join(timeout=1.0)

    sdr.deactivateStream(rx_stream)
    sdr.closeStream(rx_stream)

    meta = {
        "file": base + ".cs16",
        "samples": written,
        "rate": args.rate,
        "freq": args.freq,
        #TODO: add IQ and compass start timestamps
        "heading_final": state.heading,
        "compass_enabled": args.compass,
        "compass_bus": args.compass_bus,
        "compass_addr": hex(args.compass_addr),
        "x_offset": args.x_offset,
        "y_offset": args.y_offset,
        "x_scale": args.x_scale,
        "y_scale": args.y_scale,
        "compass_alpha": args.compass_alpha,
        "heading_correction_deg": -90
    }

    with open(json_path, "w") as f:
        json.dump(meta, f, indent=2)

    print("Done")
    print("Samples:", written)
    if args.compass:
        print("Final heading:", state.heading)
        print("Heading CSV:", heading_csv)


if __name__ == "__main__":
    main()